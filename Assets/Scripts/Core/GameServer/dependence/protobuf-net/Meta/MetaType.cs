    ...
    public class MetaType : ISerializerProxy
    {
        ...
        internal static bool GetAsReferenceDefault(RuntimeTypeModel model, Type type)
        {
            ...
            for (int i = 0; i < typeAttribs.Length; i++) {
                if (typeAttribs[i].AttributeType.FullName == "ProtoBuf.ProtoContractAttribute") {
                    if (typeAttribs[i].TryGet("AsReferenceDefault", out object tmp)) {
                        return (bool)tmp;
                    }
                }
            }
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            if (isEnum) {
                ...
                foreach(FieldInfo fld in type.GetRuntimeFields()) if (fld.IsPublic && !fld.IsStatic) {
                        list.Add(fld);
                    }
                foreach(MethodInfo mthd in type.GetRuntimeMethods()) if (mthd.IsPublic && !mthd.IsStatic) {
                        list.Add(mthd);
                    }
                foundList = list;
            }
...
#if COREFX || PROFILE259
            if (effectiveType.GetTypeInfo().IsSubclassOf(typeof(Delegate))) {
                effectiveType = null;
            }
#else
        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            if ((attrib = GetAttribute(attribs, "System.ComponentModel.DefaultValueAttribute")) != null) {
                if (attrib.TryGet("Value", out object tmp)) {
                    defaultValue = tmp;
                }
            }
            ...
            if (vm != null) {
                ...
                vm.IsMap = ignoreListHandling ? false : vm.ResolveMapTypes(out var _1, out var _2, out var _3);
        ...
        public ValueMember this[MemberInfo member] {
            get
            {
                ...
                {
                    if (x.Member == member || x.BackingMember == member) {
                        return x;
                    }
                }
                return null;
        ...
        {
            if (HasFlag(OPTIONS_EnumPassThru)) {
                return null;
            }
        ...
        internal static MetaType GetRootType(MetaType source)
        {
            ...
            try {
                ...
                MetaType tmp;
                while ((tmp = source.baseType) != null) {
                    source = tmp;
                }
                return source;
