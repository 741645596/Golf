...
namespace ProtoBuf.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
            ...
            if (metaType.IsList) {
                ...
                if (metaType.IsAutoTuple) {
                    ...
                    foreach (ValueMember member in metaType.Fields) {
                        ...
                        if (member.IsMap) {
                            member.ResolveMapTypes(out var _1, out var _2, out type); // don't need key-type
                        }
                        if (type == null) {
        ...
        {
            var coreSerializer = ValueMember.TryGetCoreSerializer(this, DataFormat.Default, itemType, out var _, false, false, false, false);
        ...
        internal IProtoSerializer TryGetBasicTypeSerializer(Type type)
        {
            ...
            if (idx >= 0) {
                return ((BasicType)basicTypes[idx]).Serializer;
            }
            ...
            lock(basicTypes) {
                ...
                if (ser != null) {
                    basicTypes.Add(new BasicType(type, ser));
                }
                return ser;
            }
        }
...
#if FEAT_COMPILER
            if (compiled) {
                return Compiler.CompilerContext.BuildSerializer(serializer, this);
            }
#endif
            ...
            if (internalsVisibleToAttribType != null) {
                ...
                foreach (MetaType metaType in types) {
                    ...
                    for (int i = 0; i < assemblyAttribsMap.Length; i++) {
                        ...
                        if (privelegedAssemblyName == assemblyName || string.IsNullOrEmpty(privelegedAssemblyName)) {
                            continue;    // ignore
                        }
